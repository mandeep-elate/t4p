...
function getAttributes($element, $props)
{
    ...
    $cleanAtts = array_filter($atts, function ($att) {
        return !!$att || $att === "0";
    });
...
function _render($nodes, $postId, $justDoHtmlAndSkipTheCss, $ssrParentProperties = [], $repeaterItemNodeId = null)
{
    ...
    foreach ($nodes as $node) {
        ...
        /** @var PropertiesData */
        $propsAndPreset = getHtmlProps($propsAndCount['propsWithDynamicDataRendered'], $presetId);
        ...
        if ($justDoHtmlAndSkipTheCss === false) {
            // Don't generate dynamic CSS for elements with presets.
            // They inherit dynamic CSS from the preset. This logic is also applied in BuilderElement.vue
            /**
             * @psalm-suppress MixedArgument
             */
            $generatedCss = getCss(
 ...
 */
function getHtmlProps($props, $presetId)
{
