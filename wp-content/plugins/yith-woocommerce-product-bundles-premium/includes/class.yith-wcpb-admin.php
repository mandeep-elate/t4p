...
if ( ! class_exists( 'YITH_WCPB_Admin' ) ) {
	...
	class YITH_WCPB_Admin {
		...
		public function woocommerce_admin_order_item_count( $count, $order ) {
			...
			if ( $counter > 0 ) {
				$non_bundled_count = intval($count) - intval($counter);
		...
		protected function get_panel_tabs() {
			$tabs = array(
				'how-to' => array(
					'title'       => __( 'How to create Product Bundles', 'yith-woocommerce-product-bundles' ), // TODO Maybe change title?
					'icon'        => 'help',
					'description' => '',
				),
			);
		...
		protected function get_panel_args() {
			return array(
				...
				'capability'       =>  apply_filters( 'ywcpb_settings_panel_capabilities', 'manage_options' ),
				'parent'           => '',
				...
				'premium_tab'      => array(
					'features' => array(
						array(
							'title'       => __( 'Add variable products to the bundle and set each item as optional or required', 'yith-woocommerce-product-bundles' ),
							'description' => __( 'In the premium version you can add product variations to the bundle and decide whether to make some products optional, allowing customers to remove them from the bundle.', 'yith-woocommerce-product-bundles' ),
						),
						array(
							'title'       => __( 'Customize every aspect of the products shown in the bundle', 'yith-woocommerce-product-bundles' ),
							'description' => __( 'Hide the product image, change the title, and insert an ad hoc description to display the bundled products in an appealing way.', 'yith-woocommerce-product-bundles' ),
						),
						array(
							'title'       => __( 'Additional pricing options', 'yith-woocommerce-product-bundles' ),
							'description' => __( 'You can set a fixed price for the bundle (without adding up the price of individual products) and apply a fixed or percentage discount. Decide whether to display the amount of money a customer can save by buying the package.', 'yith-woocommerce-product-bundles' ),
						),
						array(
							'title'       => __( 'More versatile shipping options', 'yith-woocommerce-product-bundles' ),
							'description' => __( 'With the upgraded version, you can decide if you want to dispatch the items in the package separately or all together in a single shipment, and adjust the shipping fees accordingly.', 'yith-woocommerce-product-bundles' ),
						),
						array(
							'title'       => __( 'Set limits and rules for bundle purchases', 'yith-woocommerce-product-bundles' ),
							'description' => __( 'Do your bundles need advanced configuration? You can choose the minimum and maximum number of products that can be purchased together in the bundle. Additionally, you can set a minimum number of items that should be bought to qualify for the bundle purchase. By doing so, only those who meet the conditions you set will be able to purchase it.', 'yith-woocommerce-product-bundles' ),
						),
					),
				),
			);
