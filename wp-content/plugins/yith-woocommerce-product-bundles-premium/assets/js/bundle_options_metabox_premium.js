...
jQuery( function ( $ ) {
	...
	var checkForFieldsVisibility = function () {
		if ( isBundle() ) {
			if ( 'yes' === per_items_pricing.val() ) {
				bundle_regular_price_wrap.hide();
				bundle_sale_price_wrap.hide();
				show_saving_amount_wrap.hide();

				$( '.yith_bundle_hide_if_per_item_pricing' ).hide();
				$( '.yith_bundle_show_if_per_item_pricing' ).show();
			} else {
				bundle_regular_price_wrap.show();
				bundle_sale_price_wrap.show();
				show_saving_amount_wrap.show();

				$( '.yith_bundle_show_if_fixed_pricing' ).show();
				$( '.yith_bundle_hide_if_fixed_pricing' ).hide();
			}

			if ( 'yes' === non_bundled_shipping.val() ) {
	...
	per_items_pricing.on( 'change', function () {
		if ( isBundle() && 'yes' === per_items_pricing.val() ) {
			bundle_regular_price.val( '' );
			bundle_sale_price.val( '' );
		}
		checkForFieldsVisibility();
	} );
	non_bundled_shipping.on( 'change', checkForFieldsVisibility );
	...
	$( document ).on( 'yith-wcpb-show-conditional-init', function () {
		$( '.yith-wcpb-show-conditional:not(.yith-wcpb-show-conditional--initialized)' ).hide().each( function () {
			...
			$show_conditional.addClass( 'yith-wcpb-show-conditional--initialized' );
	...
	/**
	 * Reflection fields (for easy integration support)
	 */
	( function () {
		$( '.yith-wcpb-reflection-field' ).each( function () {
			var reflection          = $( this ),
				original            = $( reflection.data( 'original' ) ),
