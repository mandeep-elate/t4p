   ...
   /*  const urlParams = new URLSearchParams(window.location.search);
    ...
    if (yearSelect) {
        yearSelect.addEventListener('change', function() {
            ...
            .then(response => response.json())
            .then(data => {
                //console.log('Fetch API get_engines response:', data);
                ...
                if (data.success && Array.isArray(data.data)) {
                    ...
                    //$(".vehicle-filter-btn").prop('disabled', false);
                }
            });
        });
    }
        ...
        const html = `
            ...
            <div class="vehicle-summary-title-action">
                <div class="vehicle-summary-header">
                    <div class="vehicle_b">
                        You've selected: ${vehicleData.make}, ${vehicleData.model}, ${vehicleData.listing}, Year: ${vehicleData.year}, Engine: ${vehicleData.engine}
                    </div>
                </div>
                </div>
                <div class="vehicle-summary-actions">
                        <a href="#" id="reset-vehicle-btn">Clear Selection</a>
                    </div>
            </div>
        `;
